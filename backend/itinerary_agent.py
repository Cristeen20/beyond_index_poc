...
def _build_planner_prompt(
    ...
    return (
        ...
        "When the `hotels` list contains more than one option, split the "
        "trip's nights across all of them — every provided hotel MUST be "
        "used on at least one night, grouped contiguously (e.g. hotel A "
        "for the first block of nights, then hotel B for the next). "
        "Choose the order that minimises travel between each night's "
        "hotel and that day's activities.\n"
        "  5. Stay within the food + activities budgets.\n"
        "  6. Include must_include items and place them at the very end of "
        "the trip — the final scheduled stop of the last day. Exclude "
        "must_exclude items.\n"
